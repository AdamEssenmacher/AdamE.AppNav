    ...
    IMauiRoutePresentationNavigator
{
    ...
    private readonly ConditionalWeakTable<Page, InvalidNativeTreeMarker> _invalidNativePages = new();
    private readonly ConditionalWeakTable<Window, InvalidNativeTreeMarker> _invalidNativeWindows = new();
    ...
    private CancellationTokenSource _nativeTreeCancellation = new();
    ...
    private long _nativeTreeGeneration;
        ...
        IMauiPresentationOperationPolicy? presentationOperationPolicy = null)
    {
        ...
        _nativeOperations = new GuardedMauiNativeNavigationOperations(
            ...
            page => _invalidNativePages.TryGetValue(page, out _),
            window => _invalidNativeWindows.TryGetValue(window, out _));
    ...
    private async Task FinalizeShutdownAsync()
    {
        ...
        finally
        {
            ...
            _nativeTreeCancellation.Dispose();
    ...
    private async Task FinalizeShutdownOnMainThreadAsync()
    {
        ...
        foreach (MauiBranchFlyoutPage flyoutPage in _trackedFlyoutPages.ToArray())
            UntrackFlyoutPage(flyoutPage);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await RunSerializedWindowMutationAsync(
            () => AttachWindowOnMainThreadAsync(window, windowId),
            cancellationToken);
    }
    ...
    private async Task AttachWindowOnMainThreadAsync(Window window, string windowId)
    {
        if (_invalidNativeWindows.TryGetValue(window, out _))
        {
        ...
        if (CurrentPage is null && presentedWindow is not null &&
            (presentedWindow.Root is not null || presentedWindow.Modals.Count > 0))
        {
            await RebuildStateFromScratchAsync(_lastState, LifecycleOperationId());
        }
    ...
    private void HandleWindowDestroying(object? sender, EventArgs e)
    {
        ...
        _externalNavigationDispatcher?.SetForegrounded(false);
        Interlocked.Increment(ref _nativeTreeGeneration);
        InvalidateNativeTreeOperations();

        _invalidNativeWindows.GetValue(window, static _ => new InvalidNativeTreeMarker());
        Page? abandonedRoot = CurrentPage;
        if (abandonedRoot is not null)
        {
            foreach (Page page in CollectLivePages(abandonedRoot))
                _invalidNativePages.GetValue(page, static _ => new InvalidNativeTreeMarker());
        }

        // Destruction ends presenter ownership of this native tree synchronously. The window still owns
        // its Page, so clearing or transferring that tree would itself be an unsafe native mutation.
        UnsubscribeWindowLifecycle(window);
        _attachedWindow = null;
        _attachedWindowId = null;
        CurrentPage = null;
        ...
        if (abandonedRoot is not null)
        {
            QueueNativeCleanupWithoutTree(
                () => ReleaseAndDiagnoseAsync(abandonedRoot).AsTask());
        }
    }

    private void InvalidateNativeTreeOperations()
    {
        CancellationTokenSource invalidatedCancellation;
        lock (_lifetimeGate)
        {
            invalidatedCancellation = _nativeTreeCancellation;
            _nativeTreeCancellation = new CancellationTokenSource();
        }

        _ = CancelAndDisposeAsync(invalidatedCancellation);
    }
    ...
    private void ValidatePlanForMaui(NavigationPlan plan)
    {
        ...
        {
            ValidateMauiNode(targetWindow.Root, "window root", allowConfiguredFlyout: true);
        }
        ...
        if (targetWindow is not null)
        {
            ...
            {
                ValidateMauiNode(modal, "window modal", allowConfiguredFlyout: false);
            }
        }
    }

    private void ValidateMauiNode(NavigationNode node, string path, bool allowConfiguredFlyout)
    {
        switch (node)
        {
            case StackNode:
            ...
            case BranchHostNode branchHost:
                if (_presentationOptions.TryGetFlyout(branchHost.Id, out _) && !allowConfiguredFlyout)
                {
                    throw new NotSupportedException(
                        $"MAUI flyout branch host '{branchHost.Id}' is only supported as the direct window root; found at {path}.");
                }

                foreach (NavigationBranch branch in branchHost.Branches)
                {
                    ValidateMauiNode(branch.Content, $"{path} branch '{branch.Id}'", allowConfiguredFlyout: false);
                }
            ...
            case ModalNode modal:
                ...
                {
                    ValidateMauiNode(modal.Content, $"{path} content", allowConfiguredFlyout: false);
                }
        ...
        if (transaction.IsInvalidated)
        {
            try
            {
                ...
                _diagnostics.Write(
                    ...
                    operationId,
                    "Presentation recovered the last committed state after native-window destruction.");
            }
            catch (Exception recoveryException)
            {
                var aggregate = new AggregateException(
                    "Presentation and destroyed-window recovery both failed.",
    ...
    {
        cancellationToken.ThrowIfCancellationRequested();
        await EnsureLogicalStateMaterializedAsync(CreateOperationId());
    ...
    private static NavigationPage? ResolveTopNavigationPage(Page? page)
    {
        ...
        return page switch
        {
            ...
            MauiBranchFlyoutPage flyoutPage when flyoutPage.Detail is not null =>
                ResolveTopNavigationPage(flyoutPage.Detail),
            _ => null
        };
    }
        ...
        bool wasResurfacedTarget = false)
    {
        ...
        return node switch
        {
            StackNode stack => await MaterializeStackAsync(
                ...
                cancellationToken,
                wasResurfacedTarget),
            BranchHostNode branchHost when _presentationOptions.TryGetFlyout(branchHost.Id, out var flyoutOptions) =>
                await MaterializeFlyoutBranchHostAsync(
                    branchHost,
                    existingPage as MauiBranchFlyoutPage,
                    flyoutOptions,
                    operationId,
                    isNavigationTarget,
                    cancellationToken,
    ...
    private async Task<Page> MaterializeFlyoutBranchHostAsync(
        BranchHostNode branchHost,
        MauiBranchFlyoutPage? existingPage,
        MauiFlyoutBranchHostOptions options,
        string operationId,
        bool isNavigationTarget,
        CancellationToken cancellationToken,
        bool wasResurfacedTarget = false)
    {
        var flyoutPage = existingPage is not null &&
                         StringComparer.Ordinal.Equals(GetHostId(existingPage), branchHost.Id)
            ? existingPage
            : new MauiBranchFlyoutPage(options);
        ...
        if (createdFlyoutPage)
        {
            ...
            WritePageLifecycle(
                NavigationDiagnosticEventKind.PresentationPageCreated,
                flyoutPage,
                "FlyoutPage was created.");
        }

        TrackFlyoutPage(flyoutPage);
        var stagedBranches = new List<MauiFlyoutBranchPresentation>(branchHost.Branches.Count);
        var retiredPages = new HashSet<Page>(ReferenceEqualityComparer.Instance);
        foreach (NavigationBranch branch in branchHost.Branches)
        {
            cancellationToken.ThrowIfCancellationRequested();
            Page? existingBranchPage = flyoutPage.FindBranchPage(branch.Id);
            ...
            ApplyBranchChrome(page, branch);
            SetBranchId(page, branch.Id);
            stagedBranches.Add(new MauiFlyoutBranchPresentation(branch.Id, branch.Title, page));
            if (existingBranchPage is not null && !ReferenceEquals(existingBranchPage, page))
                retiredPages.Add(existingBranchPage);
        }

        var desiredBranchIds = branchHost.Branches
            .Select(static branch => branch.Id)
            .ToHashSet(StringComparer.Ordinal);
        foreach (MauiFlyoutBranchPresentation stale in flyoutPage.Branches)
    ...
    private bool CanReuseNodePage(NavigationNode node, Page? existingPage)
    {
        return node switch
        {
            ...
            BranchHostNode branchHost => existingPage is TabbedPage tabbedPage &&
                                         !_presentationOptions.TryGetFlyout(branchHost.Id, out _) &&
                                         StringComparer.Ordinal.Equals(GetHostId(tabbedPage), branchHost.Id) ||
                                         existingPage is MauiBranchFlyoutPage flyoutPage &&
                                         _presentationOptions.TryGetFlyout(branchHost.Id, out _) &&
    ...
    private static Page? ResolveTopPresentedPage(Page? page)
    {
        ...
        while (current is not null)
        {
            ...
            var next = current switch
            {
                ...
                MauiBranchFlyoutPage flyoutPage when flyoutPage.Detail is not null => flyoutPage.Detail,
                _ => null
            };
    ...
    private void TrackFlyoutPage(MauiBranchFlyoutPage flyoutPage)
    {
        flyoutPage.BranchSelected -= OnFlyoutBranchSelected;
        flyoutPage.BranchSelected += OnFlyoutBranchSelected;
        if (_trackedFlyoutPages.Add(flyoutPage))
        {
            WriteHandlerLifecycle(
                NavigationDiagnosticEventKind.PresentationHandlerAttached,
                flyoutPage,
                "FlyoutPage branch selection",
                "FlyoutPage selection handler was attached.");
        }
    }

    private void UntrackFlyoutPage(MauiBranchFlyoutPage flyoutPage)
    {
        flyoutPage.BranchSelected -= OnFlyoutBranchSelected;
        if (_trackedFlyoutPages.Remove(flyoutPage))
        {
            WriteHandlerLifecycle(
                NavigationDiagnosticEventKind.PresentationHandlerDetached,
                flyoutPage,
                "FlyoutPage branch selection",
                "FlyoutPage selection handler was detached.");
        }
    }
        ...
        List<Exception> failures)
    {
        ...
        switch (page)
        {
            ...
            case TabbedPage tabbedPage:
                ...
                break;
            case MauiBranchFlyoutPage flyoutPage:
                UntrackFlyoutPage(flyoutPage);
                foreach (MauiFlyoutBranchPresentation branch in flyoutPage.Branches)
    ...
    private void OnFlyoutBranchSelected(object? sender, MauiFlyoutBranchSelectedEventArgs e)
    {
    ...
    {
        if (_suppressReconciliation || flyoutPage.FindBranchPage(branchId) is not { } branchPage)
            return Task.CompletedTask;

        _nativeOperations.SetFlyoutPresented(flyoutPage, false);
        if (StringComparer.Ordinal.Equals(flyoutPage.SelectedBranchId, branchId))
            return Task.CompletedTask;
        ...
        if (updatedWindow is not null)
        {
            RequestReconciliation(
                ...
                "Native flyout selection changed.");
        }
    ...
    private bool QueueNativeCleanup(Func<Task> cleanup) =>
        QueueNativeCleanupCore(cleanup, requiresCurrentNativeTree: true);

    private bool QueueNativeCleanupWithoutTree(Func<Task> cleanup) =>
        QueueNativeCleanupCore(cleanup, requiresCurrentNativeTree: false);

    private bool QueueNativeCleanupCore(
        Func<Task> cleanup,
        bool requiresCurrentNativeTree)
    {
        if (!TryBeginOperation())
            return false;

        long nativeTreeGeneration = Volatile.Read(ref _nativeTreeGeneration);
        _ = RunQueuedNativeCleanupAsync(cleanup, nativeTreeGeneration, requiresCurrentNativeTree);
        return true;
    }

    private async Task RunQueuedNativeCleanupAsync(
        Func<Task> cleanup,
        long nativeTreeGeneration,
        bool requiresCurrentNativeTree)
    {
        var lockTaken = false;
        try
        {
            ...
            if (requiresCurrentNativeTree &&
                nativeTreeGeneration != Volatile.Read(ref _nativeTreeGeneration))
            {
                return;
            }
    ...
    private static bool ContainsPageInStructuralTree(Page root, Page target)
    {
        ...
        return root switch
        {
            ...
            MauiBranchFlyoutPage flyoutPage => flyoutPage.Branches.Any(branch =>
    ...
    private async Task EnsureLogicalStateMaterializedAsync(string operationId)
    {
        if (CurrentPage is not null)
            return;

        WindowNode? window = _lastState.ActiveWindow;
        if (window is null || (window.Root is null && window.Modals.Count == 0))
            return;

        await RebuildStateFromScratchAsync(_lastState, operationId);
    }
    ...
    private async Task RebuildStateFromScratchAsync(NavigationState state, string operationId)
    {
        ...
        {
            _activeTransaction = null;
            CurrentPage = failedRoot;
            Exception? restoreException = null;
            try
            {
                SetAttachedWindowPage(failedRoot);
            }
            ...
            throw new AggregateException(
                "Full-state recovery failed and its previous root could not be restored.",
                recoveryException,
                restoreException);
        }
    }
    ...
    private static void CollectStructuralPages(Page page, HashSet<Page> pages)
    {
        ...
        switch (page)
        {
            ...
            case TabbedPage tabbedPage:
                ...
                break;
            case MauiBranchFlyoutPage flyoutPage:
                CollectStructuralPages(flyoutPage.Flyout, pages);
                foreach (MauiFlyoutBranchPresentation branch in flyoutPage.Branches)
        ...
        out CancellationTokenSource? linkedCancellation)
    {
        lock (_lifetimeGate)
        {
            CancellationToken nativeTreeCancellation = _nativeTreeCancellation.Token;
            linkedCancellation = callerCancellation.CanBeCanceled
                ? CancellationTokenSource.CreateLinkedTokenSource(
                    callerCancellation,
                    _shutdownCancellation.Token,
                    nativeTreeCancellation)
                : CancellationTokenSource.CreateLinkedTokenSource(
                    _shutdownCancellation.Token,
                    nativeTreeCancellation);
        }
    ...
    private sealed class MauiPresentationTransaction
    {
        ...
        private readonly long _nativeTreeGeneration;
        ...
        private readonly Dictionary<TabbedPage, TabSnapshot> _tabs =
            new(ReferenceEqualityComparer.Instance);
        private readonly Dictionary<MauiBranchFlyoutPage, FlyoutSnapshot> _flyouts =
            new(ReferenceEqualityComparer.Instance);
        ...
        public MauiPresentationTransaction(MauiNavigationPresenter presenter)
        {
            ...
            _nativeTreeGeneration = Volatile.Read(ref presenter._nativeTreeGeneration);
        ...
        public bool IsInvalidated =>
            _nativeTreeGeneration != Volatile.Read(ref _presenter._nativeTreeGeneration);
        ...
        public async ValueTask RollbackAsync()
        {
            ...
            foreach ((MauiBranchFlyoutPage flyoutPage, FlyoutSnapshot snapshot) in _flyouts)
            {
                ...
                if (snapshot.SelectedBranchId is not null)
                    _presenter._nativeOperations.SetSelectedFlyoutBranch(
                        flyoutPage,
        ...
        private void CapturePage(Page page, HashSet<Page> visited)
        {
            ...
            switch (page)
            {
                ...
                case TabbedPage tabbedPage:
                    ...
                    break;
                case MauiBranchFlyoutPage flyoutPage:
                    MauiFlyoutBranchPresentation[] branches = flyoutPage.Branches.ToArray();
                    _flyouts[flyoutPage] = new FlyoutSnapshot(
                        branches,
                        flyoutPage.Detail,
                        flyoutPage.SelectedBranchId,
        ...
        private sealed record FlyoutSnapshot(
            MauiFlyoutBranchPresentation[] Branches,
            Page? Detail,
    ...
    private sealed class InvalidNativeTreeMarker
    {
    }
