...
internal static class NavigationStateValidator
{
    ...
    public static void ValidateState(NavigationState? state, string source)
    {
        ...
        foreach (var window in RequireListItems(state.Windows, $"{source}.Windows"))
        {
    ...
    private static void ValidateWindow(WindowNode window, string path)
    {
        ...
        foreach (var modal in RequireListItems(window.Modals, $"{path}.Modals"))
        {
    ...
    private static void ValidateStack(StackNode stack, string path)
    {
        ...
        foreach (var entry in RequireListItems(stack.Entries, $"{path}.Entries"))
        {
    ...
    {
        var branches = RequireListItems(branchHost.Branches, $"{path}.Branches");
        if (branches.Count == 0)
        {
